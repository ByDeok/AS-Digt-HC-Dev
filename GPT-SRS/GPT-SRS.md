...
|----|------------------------|-------------|----------------|-----------------|----------------------------|
| REQ-FUNC-001 | 시스템은 최근 6개월 간 Senior 사용자 헬스 데이터를 통합하여 1장 의사용 요약 리포트를 생성할 수 있어야 한다. | Story 1 (AC1) | F2 | Must | Given 데이터 연동 완료, When 1장 리포트 생성 요청 시, Then p95 생성 시간 ≤ 3초(REQ-NF-001)이고 활동·심박·혈압·체중이 기간·기기·단위 기준으로 표준화되어 표기된다(필드 완결성 ≥ 95%, REQ-NF-005). |
...
| REQ-FUNC-005 | 시스템은 1장 리포트에 대해 접근성 모드(대글자/고대비/스크린리더)를 제공해야 한다. | Story 1 (AC3) | F10 | Should | Given 접근성 모드 활성, When 리포트 열람 시, Then 대글자/고대비 테마 적용 및 모든 인터랙티브 요소에 스크린리더 레이블이 존재하여 라벨 누락 0건(REQ-NF-008). |
| REQ-FUNC-006 | 시스템은 하루 사용자당 1~3개의 행동 카드를 생성해야 하며, 이 범위를 초과하는 카드를 생성해서는 안 된다. | Story 2 (AC1) | F3, F9 | Must | Given 알림 창구 설정 완료, When 일일 행동 카드 생성 시, Then 동일 사용자 기준 일일 제안 개수 ∈ [1, 3]이며 0개 또는 4개 이상 제안되는 비율 = 0%. |
...
| REQ-NF-009 | Product KPI | 행동 카드 완성도 | D1 행동 완료율 ≥ 25%, W1 주간 3일 이상 달성률 ≥ 50% | Story 2 AC2; PRD 1 보조 KPI |
...
|----------|------------|----------------|--------------------------------------|-----------------------|
| Story 1 | Senior가 외래에서 제시할 1장 의사용 요약을 원함 | F2, F7, F6 | REQ-FUNC-001, 002, 003, 004, 005, 019; REQ-NF-004~007, 005, 006, 008, 023, 024 | TC-001~TC-015 |
| Story 2 | Senior가 하루 1~3개의 실행 가능한 행동을 원함 | F3, F9 | REQ-FUNC-006, 007, 008, 009; REQ-NF-002, 003, 009, 010 | TC-016~TC-025 |
...
| API-008 | POST | `/api/v1/actions/daily` | 오늘 1~3개 행동 카드 생성 | REQ-FUNC-006, 008; REQ-NF-009 |
...
**요약**: 위 SRS는 주어진 PRD를 단일 Source of Truth로 삼아, Story→REQ-FUNC, KPI→REQ-NF 매핑, 인터페이스/데이터 모델/시퀀스 다이어그램을 포함하는 ISO 29148 준수 구조로 작성되었으며, 모든 요구사항은 ID 기반으로 테스트 및 추적 가능하도록 명세되었다.