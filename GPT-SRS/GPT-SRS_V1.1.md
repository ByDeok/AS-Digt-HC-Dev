...
### 1.5 Assumptions & Constraints (기술 제약 – Platform & Stack)

아래 항목은 **설계 상 제약(Design Constraints)**으로, 상세 선택·변경 이력은 ADR에 기록한다.

* **C-TEC-001 (Frontend Platform)**

  * 모든 프론트엔드 서비스는 **웹 기반 SPA**를 사용한다. 초기 구현은 Vite 환경 기반 React.js를 기본 선택으로 하며, 세부 번들러·프레임워크 선택 및 변경은 관련 ADR을 따른다.
* **C-TEC-002 (Backend Platform)**

  * 모든 핵심 백엔드 서비스는 **JVM 기반(Java 17 호환)** 스택을 사용한다. 초기 프레임워크는 Spring Boot 3.x를 기본으로 하며, 변경 시 ADR에 기록한다.
* **C-TEC-003 (Database)**

  * 트랜잭션성 데이터 저장소는 **MySQL 9.x InnoDB 엔진** 또는 동급 RDBMS를 사용하고, 유니코드 확장 문자 인코딩을 기본으로 한다.
* **C-TEC-004 (Document Generation / LLM Integration)**

  * 문서 자동생성·LLM 보조 기능은 Python 기반 서비스(예: FastAPI + LLM 프레임워크)를 사용한다. 초기 구현은 LangChain + FastAPI를 가정하되, 구체 라이브러리는 ADR에 명시한다.
* **C-TEC-005 (LLM Gateway)**

  * LLM 호출은 OpenAI API 또는 사내 LLM Gateway를 사용하며, 민감 데이터 처리 정책은 보안·규제 요구(REQ-NF-016, 036, 038)에 따른다.
* **C-TEC-006 (Service-to-Service Communication)**

  * MSA 서비스 간 통신은 REST(OpenAPI 3.x 기반) 또는 gRPC로 제한한다. 선택 기준 및 예외는 ADR에 기록한다.

### 1.6 Operating Environment & Regulatory Context (운영 환경 및 규제)

* **사용자/디바이스 환경**

  * 주요 사용자는 **50~75세 시니어** 및 보호자이며, 다음 환경을 가정한다.

    * 중저가 안드로이드 단말(메모리 제약, 구형 CPU 포함), 최신 iOS 단말 혼재.
    * 네트워크 품질: 실내 Wi-Fi + 불안정 LTE 환경(간헐적 단절, 고지연).
    * 화면 크기: 5~7인치 스마트폰 중심.
  * 앱은 최소 **Android 10**, **iOS 15** 이상을 공식 지원 대상으로 한다.

* **운영·규제 환경 (대한민국 기준)**

  * 개인정보보호법(PIPA), 의료법 등 관련 법령을 준수해야 하며, 의료기관 연동 시 병원 IT·보안 가이드를 따른다.
  * 건강 데이터는 원칙적으로 **국내 리전**에 저장하며, 국외 이전 시 별도 동의/계약이 필요하다(REQ-NF-038 참조).
  * 동의/위임/감사로그는 법적 보존 의무 기간을 충족하도록 관리한다(REQ-NF-018, 036).
...
* **모바일 앱(시니어/보호자 공용 앱)**
  ...
  * 플랫폼: iOS/Android (세부 플랫폼·최소 버전: Android 10+, iOS 15+).
...
### 4.0 Functional Overview (Fn 카탈로그)

기능 요구사항에서 참조하는 `Fn` 식별자는 다음과 같이 정의한다.

| Fn ID | 기능 이름         | 개요                                         |
| ----- | ------------- | ------------------------------------------ |
| F1    | 온보딩 플로우       | 신규 사용자 가입·인증·동의·디바이스/병원 연동을 포함한 3분 온보딩 플로우 |
| F2    | 1장 의사용 요약 리포트 | 최근 6개월 헬스 데이터 통합·표준화 및 1페이지 리포트 생성/조회/내보내기 |
| F3    | 행동 코칭 엔진      | 하루 1~3개 행동 카드 생성·상태 관리 및 KPI 집계            |
| F4    | 가족 보드 & 초대    | 가족 보드 생성·초대·역할 관리 및 일정/복약 공유               |
| F5    | 병원 포털 연동      | 병원/검진 포털 연동 및 파일 업로드 기반 대체 연동              |
| F6    | 디바이스 연동/정합성   | 최소 2종 디바이스 데이터 연동 및 멀티디바이스 정합성 유지          |
| F7    | 맥락/기기/기간 태깅   | 지표별 기간·기기·결측 등 맥락 태그 관리 및 리포트 표출           |
| F8    | 보안·위험기반 인증    | 권한·대리접근·위험기반 2FA·감사로그 관리                   |
| F9    | 알림/오프라인 큐     | 알림 발송, 야간 침묵, 오프라인 큐잉·재시도 로직               |
| F10   | 접근성(A11y)     | 대글자/고대비/스크린리더·포커스 관리 등 접근성 모드              |
...
> 주: 아래에서 참조하는 비기능 요구사항 ID는 4.2 절에 정의된 내용과 일관되도록 수정되어 있다.
...
| REQ-FUNC-005 | 시스템은 1장 리포트에 대해 접근성 모드(대글자/고대비/스크린리더 및 문맥 설명 툴팁)를 제공해야 한다.                | Story 1 (AC3)                 | F10       | Should          | Given 접근성 모드 활성, When 리포트 열람 시, Then 대글자/고대비 테마 적용 및 모든 인터랙티브 요소에 스크린리더 레이블이 존재하여 라벨 누락 0건(REQ-NF-008, 037)이고, 핵심 용어/지표에 대한 문맥 설명 툴팁 커버리지가 ≥ 90%(REQ-NF-031).     |
| REQ-FUNC-006 | 시스템은 하루 사용자당 1~3개의 행동 카드를 생성해야 하며, 이 범위를 초과하는 카드를 생성해서는 안 된다.             | Story 2 (AC1)                 | F3, F9    | Must            | Given 알림 창구 설정 완료, When 일일 행동 카드 생성 시, Then 동일 사용자 기준 일일 제안 개수 ∈ [1, 3]이며 0개 또는 4개 이상 제안되는 비율 = 0%이고, 행동 카드 관련 오경보율 측정이 가능하며(REQ-NF-032), PRD 목표 기준으로 < 2%를 지향한다. |
...
| REQ-FUNC-008 | 시스템은 행동 카드 수락/완료 이벤트를 기록하여 D1 완료율과 W1 주간 3일 이상 달성률을 계산할 수 있어야 한다.         | Story 2 (AC2)                 | F3        | Must            | Given 행동 카드 노출, When 사용자가 수락 또는 완료하면, Then 수락/완료 시각 및 상태가 서버에 기록되고, 시스템은 D1/W1 지표를 집계·조회 가능한 형태로 제공한다(REQ-NF-009, 033).                                           |
...
| REQ-FUNC-012 | 시스템은 가족 보드의 일정/약 변경 사항을 모든 구성원에게 동기화해야 한다.                                | Story 3 (AC2)                 | F4        | Should          | Given 가족 보드 일정/약 변경, When 다른 구성원이 보드를 열람하면, Then 변경 전파 지연 ≤ 60초(REQ-NF-003)이며, 동시 변경 충돌 시 최신 기준으로 병합하고 변경 이력·감사로그를 기록한다(REQ-NF-018).                              |
| REQ-FUNC-013 | 시스템은 민감정보(건강 데이터, 권한 설정 등) 접근 시 이상 위치/시간 접근을 탐지하고 추가 인증을 요구해야 한다.         | Story 3 (AC3)                 | F8        | Should          | Given 비정상 위치/시간 접근 탐지, When 보호자 또는 시니어가 민감정보에 접근하면, Then OTP/바이오 인증 중 하나를 요구하고, 인증 실패 시 접근을 차단하며 알림을 발송한다(REQ-NF-017, 018).                                       |
...
| REQ-FUNC-015 | 시스템은 신규 사용자의 가입→인증→권한 설정→디바이스/병원 연동을 하나의 온보딩 플로우로 제공해야 한다.                | Story 4 (AC1)                 | F1, F5    | Must            | Given 신규 설치, When 온보딩 플로우 실행 시, Then 위 4단계를 순차 또는 논리적 그룹으로 제공하고, 시스템은 온보딩 소요 시간과 완료 여부를 기록하여 KPI 집계가 가능해야 한다(REQ-NF-012, 013).                                    |
| REQ-FUNC-016 | 시스템은 온보딩 각 단계에서 되돌아가기 및 중간 저장 기능을 제공해야 한다.                                | Story 4 (AC2)                 | F1        | Must            | Given 온보딩 진행 중, When 사용자가 이전 단계로 이동 또는 앱을 종료했다가 재시작하면, Then 마지막 완료 단계 이후부터 재개 가능하며, 단계별 이탈률 측정이 가능하다(REQ-NF-013).                                                 |
| REQ-FUNC-017 | 시스템은 온보딩 화면에 대해 스크린리더 라벨 완전성 및 포커스 관리가 보장되는 접근성 모드를 제공해야 한다.              | Story 4 (AC3)                 | F10       | Should          | Given 접근성 설정 활성, When 온보딩 화면 탐색 시, Then 모든 포커스 가능한 요소에 라벨이 존재하고 포커스 트랩이 없으며, 폼 오류 발생 시 오류 힌트가 100% 노출된다(REQ-NF-008, 037).                                         |
...
**제품 KPI 값은 “시스템이 측정·집계해야 할 대상”이며, 목표 수치는 PRD 기준 참고값으로 명시한다.**
...
| REQ-NF-008 | Usability/Accessibility     | 접근성 품질(기본)           | 접근성 모드에서 스크린리더 라벨 누락 0건, 온보딩/리포트 화면 포커스 트랩 0건, 오류 힌트 노출율 100%                                    | Story 1 AC3; Story 4 AC3; F10     |
| REQ-NF-009 | Product KPI/Instrumentation | 행동 카드 KPI 측정         | 시스템은 D1 행동 완료율·W1 주간 3일 이상 달성률을 집계·조회 가능해야 한다. PRD v0 기준 초기 성공 목표: D1 ≥ 25%, W1 ≥ 50%            | Story 2 AC2; PRD 1 보조 KPI         |
...
| REQ-NF-012 | Product KPI/Instrumentation | 온보딩 퍼널 측정            | 시스템은 온보딩 p50 완료 시간 및 성공률을 측정·집계해야 한다. PRD v0 기준 목표: p50 ≤ 180초, 전체 성공률 ≥ 65%                     | Story 4 AC1; PRD 1 보조 KPI         |
| REQ-NF-013 | Product KPI/Instrumentation | 온보딩 단계별 이탈률 측정       | 각 온보딩 단계별 이탈률을 집계·조회 가능해야 하며, PRD v0 기준 설계 목표는 각 단계별 이탈률 ≤ 15%                                   | Story 4 AC2                       |
...
| REQ-NF-021 | Monitoring                  | 모니터링 대시보드            | 지연/오류/가용성·비용·핵심 KPI 지표를 통합 모니터링 대시보드에서 실시간(≤1분 지연)으로 제공                                          | PRD 5, L129-131                   |
| REQ-NF-022 | Alerting                    | 알림·에스컬레이션            | 임계치 초과 후 5분 이내 온콜 알림 발송, 동일 이슈 반복 시 단계적 에스컬레이션                                                   | PRD 5, L129-131                   |
| REQ-NF-023 | Product KPI/Instrumentation | 북극성 KPI – 1장 리포트 채택률 | 시스템은 외래 현장 1장 리포트 채택률을 집계·조회 가능해야 하며, PRD v0 기준 파일럿 목표는 ≥ 40% (기준선 10%)                          | PRD 1, L9-13; 실험 3                |
| REQ-NF-024 | Product KPI/Instrumentation | 1장 리포트 현장 성과         | 시스템은 의사/환자 평점(5점 만점) 및 의사결정 시간 지표를 수집·집계할 수 있어야 하며, PRD v0 기준 목표는 평점 ≥ 4.2, 의사결정 시간 -20% 이상 단축   | PRD 1, L40-43; PRD 7, L162-167    |
| REQ-NF-025 | Product KPI/Instrumentation | 가족 대리접근 설정률          | 시스템은 가족 대리접근 설정률을 집계·조회 가능해야 하며, PRD v0 기준 목표는 ≥ 50% (기준선 20%)                                   | PRD 1, L28-31                     |
| REQ-NF-026 | Product KPI/Instrumentation | 90일 유지율              | 시스템은 D90 유지율을 집계·조회 가능해야 하며, PRD v0 기준 목표는 ≥ 35% (기준선 20%)                                       | PRD 1, L32-35                     |
| REQ-NF-027 | Product KPI/Instrumentation | 복약 순응도               | 시스템은 복약 완료율을 집계·조회 가능해야 하며, PRD v0 기준 파일럿 목표는 완료율 +15%p 향상                                       | PRD 1, L36-39                     |
...
| REQ-NF-030 | Experimentation             | 실험 성공 기준             | 실험 1/2/3 각각 PRD 7에 정의된 성공 기준을 측정·판별할 수 있어야 하며, 기준 충족 시 해당 기능·설계안은 채택 후보로 간주                      | PRD 7, L148-167                   |
| REQ-NF-031 | Usability/Accessibility     | 1장 리포트 문맥 설명 툴팁 커버리지 | 1장 리포트 내 핵심 지표·용어에 대한 문맥 설명 툴팁 커버리지 ≥ 90%                                                        | Story 1 AC3                       |
| REQ-NF-032 | Product KPI/Instrumentation | 행동 카드 오경보율           | 시스템은 행동 카드 알림 중 오경보율을 산출할 수 있어야 하며, PRD v0 기준 설계 목표는 < 2%                                        | Story 2 AC1; PRD 7, L155-160      |
| REQ-NF-033 | Product KPI/Instrumentation | 행동 카드 장기 KPI (제품 목표) | 시스템은 장기 제품 KPI로서 D1 행동 완료율·W1 3일 이상 달성률을 추적·집계해야 하며, PRD v0 장기 목표는 D1 ≥ 60%, W1 ≥ 50%            | PRD 1, L20-23; PRD 7, L155-160    |
| REQ-NF-034 | Product KPI/Instrumentation | 월간 ‘의사용 1장’ 생성률      | 시스템은 월간 고유 활성 사용자 대비 1장 리포트 생성 사용자 비율을 집계해야 하며, PRD v0 기준 목표는 ≥ 50% (기준선 20%)                    | PRD 1, L24-27                     |
| REQ-NF-035 | Reliability/BCP             | 데이터 복구 목표(RPO/RTO)   | 중요 데이터(RDB 기준) RPO ≤ 10분, RTO ≤ 30분                                                              | Derived (BCP 요구)                  |
| REQ-NF-036 | Security/Compliance         | 데이터 보존·삭제 정책         | AuditLog는 최소 3년 보관. 계정 삭제 요청 시 법적 보존 의무 범위 외 데이터는 30일 이내 논리적 삭제 또는 비식별화 처리                       | Derived (규제·감사 요구)                |
| REQ-NF-037 | Accessibility               | 접근성 표준 준수            | 온보딩, 1장 리포트, 행동 카드, 가족 보드 플로우는 WCAG 2.1 AA 수준 중 적용 가능한 항목을 충족하도록 설계하며, 적용/제외 항목은 A11y 가이드 문서에 명시 | Derived (A11y 정책)                 |
| REQ-NF-038 | Security/Data Locality      | 데이터 지역성 및 LLM 사용     | 건강 데이터는 국내 리전 내 저장을 원칙으로 하며, LLM 호출 시 민감 필드는 마스킹 또는 비동기 비저장 모드로만 사용                              | Derived (데이터 지역성·LLM 정책)          |
...
| -------- | -------------------------------- | ----------- | -------------------------------------------------------------------------------------------------- | ------------------ |
| Story 1  | Senior가 외래에서 제시할 1장 의사용 요약을 원함   | F2, F7, F6  | REQ-FUNC-001, 002, 003, 004, 005, 019; REQ-NF-004~007, 005, 006, 008, 023, 024, 031, 034, 037, 038 | TC-001~TC-015      |
| Story 2  | Senior가 하루 1~3개의 실행 가능한 행동을 원함   | F3, F9      | REQ-FUNC-006, 007, 008, 009; REQ-NF-002, 003, 009, 010, 021, 032, 033                              | TC-016~TC-025      |
| Story 3  | Caregiver가 역할 분담 가능한 가족 보드를 원함   | F4, F5, F8  | REQ-FUNC-010, 011, 012, 013, 014; REQ-NF-003, 011, 016, 017, 018, 021, 025, 036, 038               | TC-026~TC-038      |
| Story 4  | New user가 3분 내 온보딩 완료 후 첫 가치를 원함 | F1, F5, F10 | REQ-FUNC-015, 016, 017, 018; REQ-NF-001, 002, 008, 012, 013, 021, 022, 026, 035, 037               | TC-039~TC-050      |
...
| API-009 | PATCH  | `/api/v1/actions/{id}`                            | 행동 카드 상태(수락/완료) 업데이트          | REQ-FUNC-008, 009; REQ-NF-009           |
| API-010 | POST   | `/api/v1/family-board/invitations`                | 가족 보드 초대 발송                   | REQ-FUNC-010, 011; REQ-NF-011, 018      |
...
| API-017 | GET    | `/internal/metrics`                               | 성능·KPI·비용 지표 조회(모니터링 시스템 연동)  | REQ-NF-021, 022, 009~027, 032~034       |
| API-018 | POST   | `/internal/alerts/test`                           | 알림·에스컬레이션 구성 검증용 테스트 트리거      | REQ-NF-022                              |

### 6.2 Entity & Data Model

주요 엔터티 및 데이터 모델은 아래와 같이 구조화한다. (Type은 논리적 타입 기준이며, 실제 물리 스키마는 설계 단계에서 확정한다.)
...
| board_id          | UUID                             | 보드 ID       | FK(FamilyBoard.board_id) |
...
**요약 1.0**: 
위 SRS 확정본은 기존 SRS에 대해 PRD의 모든 Story·AC·KPI를 재점검하여, 누락되었던 툴팁 커버리지·오경보율·행동 KPI·월간 리포트 생성률 등을 추가 NFR로 반영하고, 관련 REQ-FUNC·Traceability·API 매핑을 보완함으로써 ISO 29148 기반 Definition of Done을 충족한다.

**요약 1.1**:
이 Revision 1.1 SRS는 기존 버전에서

* REQ-NF 참조 오류 수정(특히 리포트 생성 성능),
* 기술 스택을 “요구사항”이 아닌 “제약·ADR 연동”으로 정리,
* Product KPI 값을 **측정·집계 요구**로 정리하고 목표 수치는 PRD 기준 참고값으로 분리,
* RPO/RTO, 데이터 보존·지역성, A11y 표준(WCAG) 명시,
* Fn 카탈로그·운영환경·규제 문맥·모니터링용 내부 API 추가

까지 반영해서, 설계/구현/테스트 팀이 그대로 기준선으로 써도 무리 없도록 정리한 상태다.
